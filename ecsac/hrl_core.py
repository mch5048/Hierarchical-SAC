...
def mlp_deterministic_policy(stt, goal, sub_goal, aux, activation=tf.nn.relu, hidden_sizes=(512,256,256), output_activation=tf.nn.tanh, kernel_initializer=ortho_init(init_scale)):
    """ policy for high-level manager, TD3 policy
    """
    batch_size = sub_goal.shape.as_list()[0]
    sg_dim = sub_goal.shape.as_list()[-1]
    net = mlp(tf.concat([stt,goal], axis=-1), list(hidden_sizes), activation=activation, kernel_regularizer=None, kernel_initializer=kernel_initializer)
...
def cnn_controller_actor_critic(stt, obs, goal, act, aux, action_space=None, hidden_sizes=(400,300), activation=tf.nn.relu, 
